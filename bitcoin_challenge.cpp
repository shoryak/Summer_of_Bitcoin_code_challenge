...
/*     vector<transaction> transactions_to_include_in_block;  // this stores the final transactions that we will include in the block
   
    ll profit=0;  // profit is the sum of fee of all transactions in 

    while(capacity > 0){     // we will repeat the same process till capacity is positive and atleast one transaction was included in previous iteration
        int number_included_in_this_itereation=0;

        vector<transaction> not_included_in_this_itereation;
        
        // lets iterate over all transactions that are not yet included in the block and are sorted according to priority 
        // where transaction with greater priority comes first
        for(auto transaction : transactions){

            if(transaction.weight > capacity){ //if weight of the current transaction is greater than current capacity we just ignore it to save time
                continue;
            }
            
            // now for the transaction we check if all its parents are already included in the block
            bool all_parents_already_included = true;

            for(auto parent : transaction.parents){  // itereate over all parents 
                
                if(!is_included[parent]){          // if any of them is not included already 
                    all_parents_already_included= false;   // then make the boolean false and break the loop

                    break;
                }
            }

            // now if all parents are already included and current capacity is greater than weight of the transaction we include it in our final transaction list
            if(all_parents_already_included && capacity>=transaction.weight){
                profit+= transaction.fee; // increasing profit

                capacity-= transaction.weight;  // decreasing the remaining capacity 

                number_included_in_this_itereation++; // increasing the count of transactions included in this iteration

                is_included[transaction.txid]=true;  // marking it included on our global marker 

                transactions_to_include_in_block.push_back(transaction); // adding it to final list of selected transactions
            }
            else{           // else we add it in the not_included_list 
                not_included_in_this_itereation.push_back(transaction);
            }

        } 
        if(number_included_in_this_itereation==0){  // if no transaction is included in this iteration than we end the process

            break;
        }
        // assigning the left transactions to transactions vector so they can be included in the next iteration
        transactions = not_included_in_this_itereation;sum_of_reward_of_all_transactions;

    }

    // Now we have a list of transactions to include 


    cout<<"sum_of_reward_of_all_transactions            | " <<sum_of_reward_of_all_transactions<<endl; 
    
    cout<<"\nnumber of transactions included              | "<<transactions_to_include_in_block.size()<<endl; // number of transactions included
    ...
    for(auto x : transactions_to_include_in_block){
        s.insert(x.txid);
        cout<<x.txid<<endl;
    ...
    for(auto i : transactions){
        if(indegree[i.txid]==0ll){
           ...
           /*  if(p< priori[i.txid]){ // this is where i say that if max priori among all children is less than priori of the parent just dont include the effect of children while inserting in priority queue
                p=0;
                c=1;
            } */
